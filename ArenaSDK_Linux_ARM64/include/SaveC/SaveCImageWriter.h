 ...
 *  - Returns SC_ERR_SUCCESS (0) on success
*
 * <B> saveWriterSetPlyAndConfig </B> changes the image writer to save images as
 * ply data. It also switches the extension to '.ply'. Uses filterPoints as
 * parameter to filter NaN points
*/
 ...
 *  - Returns SC_ERR_SUCCESS (0) on success
*
 * <B> saveWriterSetPlyAndConfigExtended </B> changes the image writer to save
 * images as ply data. It switches the extension to '.ply' and uses <B>
 * savePlyParams </B> information to configre the save data. The
 * param.filterPoints option will filter NaN points in the output data.  The
 * params.scale, and params.offset(A/B/C) will be applied to the output PLY file
 * accordingly also.  Finally the params.isSigned value lets the library know if
 * data passed in is signed or unsigned.
*/
