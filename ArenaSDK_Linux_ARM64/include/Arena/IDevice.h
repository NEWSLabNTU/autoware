...
namespace Arena
{
	...
	public:
		 ...
		 * image data is needed for longer (i.e. for
		...
		*  - Arena::IDevice::StopStream
		*/
		virtual void WaitForNextLeader(uint64_t timeout) = 0;

		/**
		* @fn virtual void ResetWaitForNextLeader()
		*
		* @return
		*  -Type: none
		*
		* <B> ResetWaitForNextLeader < / B> clears any pending flag
		* for a received leader event.
		*
		* @warning
		*  -May throw GenICam::GenericException or other derived exception.
		*
		* @see
		*  -Arena::IDevice::WaitForNextLeader
		*  -Arena::IDevice::StartStream
		*  -Arena::IDevice::StopStream
		*/
		virtual void ResetWaitForNextLeader() = 0;
		 ...
		 * interface node map (GenApi::INodeMap), used to access interface-related
		 * nodes (GenApi::INode).
		 *
		...
		*/
		...
		/**
		 * @fn virtual void DownloadXml()
		 *
		 * @return
		 *	- none
		 *
		 * <B> DownloadXml </B> downloads the on-camera XML file to the relative
		 * path <serial>.xml.
		 */
		virtual void DownloadXml() = 0;
