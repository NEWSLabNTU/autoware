...
//    purpose, without actually creating an HDR image.
...
// Image timeout
//    Timeout for grabbing images (in milliseconds). Have the timeout somewhat
//    larger than the highest exposure time to avoid timing out.
...
//    exposure time, and one with a lower exposure time.
struct HDRImage
{
public:
...
//    Continually check until trigger is armed. Once the trigger is armed, it is
...
void AcquireHDRImages(Arena::IDevice* pDevice)
{
	...
	//    in order to check for existence, readability, and writability only once
	//    before the stream.
	...
	// get max and min exposure times and ensure the set of exposure times is within
	// this range
	...
	// if longest exposure times is not within the exposure time range, set
	// longest exposure time to max value and set the remaining exposure times to
	// half the value of the state before
	...
	//    into an HDR image with HDR algorithms
