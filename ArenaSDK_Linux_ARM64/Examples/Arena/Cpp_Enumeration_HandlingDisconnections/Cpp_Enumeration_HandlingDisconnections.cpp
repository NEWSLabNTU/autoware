...
//    simplicity. This structure could also be placed in a class.
		...
		while (g_isRunning && !g_pDevice)
		{
			...
			if (deviceInfos.size() > 0)
			{
				...
				if (it != deviceInfos.end())
				{
					...
					//    forcing an IP address whenever the device is updated
					//    (see Cpp_ForceIP), (3) running ArenaConfig to configure
					//    the adapter settings, or (4) setting up a persistent IP
					//    on the camera using IPConfigUtility
...
void AcquisitionThread(Arena::ISystem* g_pSystem)
{
	...
	Arena::SetNodeValue<bool>(g_pDevice->GetTLStreamNodeMap(), "StreamAutoNegotiatePacketSize", true);

	// enable stream packet resend
	Arena::SetNodeValue<bool>(g_pDevice->GetTLStreamNodeMap(), "StreamPacketResendEnable", true);
	...
	while (g_isRunning)
	{
		...
		try
		{
			if (!g_pDevice)
			{
				...
				//    When the device has been disconnected, the enumeration
				//    thread is notified to start running, while the acquisition
				...
				//    Check that the device has been reconnected in case the
...
int main()
{
	...
	try
	{
		...
		g_serialNumber = selectedDeviceInfo.SerialNumber();
		g_subnetMask = selectedDeviceInfo.SubnetMask();
		g_pDevice = g_pSystem->CreateDevice(selectedDeviceInfo);
