...
//    will cycle through 3 sequencer states, with each one using a different exposure
//    time. The images could then be used to generate an HDR image. Much like a
//    state machine, to use the sequencer we must initialize each set
...
//    less light is available, the higher the exposure time. Storing multiple
//    values so that each consecutive state can be saved with varying exposure
//    times.
...
void SetSequencerSet(GenApi::INodeMap* nodeMap, int64_t setNumber, double exposureTime, int64_t pathNextSet, GenICam::gcstring triggerSource)
{
	...
	// select the path we want it to follow from this set to the next set; there can
...
void AcquireImagesUsingSequencer(Arena::IDevice* pDevice)
{
	...
	// if the longest exposure time is not within the exposure time range, set
	// the longest exposure time to max value and set the remaining exposure times to
