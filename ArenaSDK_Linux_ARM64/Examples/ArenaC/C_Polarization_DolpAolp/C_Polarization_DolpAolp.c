...
//    formats. Specifically, this example retrieves a 2-channel DoLP, AoLP
...
#define FILE_NAME "Images/C_Polarization_DolpAolp.jpg"
...
AC_ERROR ConvertDoLPAoLPToPixelFormat(acDevice hDevice)
{
	...
	printf("%sSet PolarizedDolpAolp_Mono8 to pixel format\n", TAB1);

	err = acNodeMapSetEnumerationValue(hNodeMap, "PixelFormat", "PolarizedDolpAolp_Mono8");
	if (err != AC_ERR_SUCCESS)
		return err;
	...
	//    manually convert from HSV to the desired pixel format: algorithm
	...
	{
		printf("\n\nThis example requires PolarizedDolpAolp_Mono8 pixel format");
		return err;
	}
	...
	err = saveWriterCreate(saveWidth, saveHeight, dstBpp, &hWriter);
	if (err != AC_ERR_SUCCESS)
		return err;
	...
	err = acImageFactoryCreate(pOutput, dstDataSize, saveWidth, saveHeight, PIXEL_FORMAT, &hCreate);
	if (err != AC_ERR_SUCCESS)
		return err;
...
int main()
{
	printf("C_Polarization_DolpAolp\n");
	AC_ERROR err = AC_ERR_SUCCESS;
