...
AC_ERROR AcquireImageAndInterpretData(acDevice hDevice)
{
	...
	if (strstr(deviceModelName, "HLT") != NULL || strstr(deviceModelName, "HTP") != NULL)
	{
		isHelios2 = true;
	}
	...
	savePlyParams plyParams =
	{
		...
		isSignedPixelFormat,         
		(float)scaleX,            // using scaleX as scale since all scales = 0.25f. Cast to float since saveWriterSetPlyAndConfigExtended will expect it as float
		(float)offsetA,           // offsetA cast to float since saveWriterSetPlyAndConfigExtended will expect it as float
		(float)offsetB,           // offsetB cast to float since saveWriterSetPlyAndConfigExtended will expect it as float
